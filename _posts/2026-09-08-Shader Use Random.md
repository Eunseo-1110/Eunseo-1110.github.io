...
- 랜덤 챕터의 예제 코드와 화면으로 랜덤한 가로줄이 랜덤한 색상으로 랜덤 속도로 이동하는 화면을 구성해봤다.
    ...
    window.runGLSL("random-line", `
        precision mediump float;uniform vec2 u_resolution;uniform vec2 u_mouse;uniform float u_time;vec3 hsb2rgb( in vec3 c ){vec3 rgb = clamp(abs(mod(c.x*6.0+vec3(0.0,4.0,2.0), 6.0)-3.0)-1.0, 0.0, 1.0 ); rgb = rgb*rgb*(3.0-2.0*rgb);return c.z * mix(vec3(1.0), rgb, c.y); }float random (in float v) { return fract(sin(v * 10000.0)); }float random (in vec2 _st) {return fract(sin(dot(_st.xy,vec2(12.9898,78.233)))*43758.5453123);}float line(vec2 st, float r, float t){float p = floor(st.x + t);return step(r, random(mod(p, 50.0)));} void main(){ vec2 st = gl_FragCoord.xy/u_resolution.xy; st *= vec2(10.0, 50.0);vec2 ipos = floor(st);  vec2 fpos = vec2(st); float randY = random(1.0 + ipos.y);float t = u_time * 10.0 * randY;vec3 lineColor = hsb2rgb(vec3(random(1.25 + ipos.y), 1.0, 1.0));vec3 color = vec3(0.0); color = mix(vec3(1.0), lineColor,line(vec2(st.x, st.y), random(vec2(ipos.y, 12.12)), -t)); gl_FragColor = vec4(color,1.0);}
    `);
    </script>
